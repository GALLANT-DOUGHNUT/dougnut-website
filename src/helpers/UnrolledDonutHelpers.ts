...
const redrawSocialGraphSegments =
  (
    geometry: UnrolledGeometry
  ) => {
    const {
...
const redrawEcologicalGraphSegments =
  (
    geometry: UnrolledGeometry
  ) => {
    const {
...
const createGraphLabels = (
  ...
  group
    ...
    .attr(
      "id",
      "global-social-foundation-label"
    )
    .attr(
      "x",
      0.25 * windowWidth
  ...
  group
    ...
    .attr(
      "id",
      "local-ecological-ceiling-label"
    )
    .attr(
      "x",
      0.75 * windowWidth
  ...
  group
    ...
    .attr(
      "id",
      "global-ecological-ceiling-label"
    )
    .attr(
      "x",
      0.25 * windowWidth
    ...
    .text(
      "GLOBAL ECOLOGICAL CEILING"
    )
}

const redrawGraphLabels = (
  geometry: UnrolledGeometry
) => {
  const {
    windowHeight,
...
const redrawSocialIcons = (
  ...
  data.forEach((domain) => {
    const imgIconId =
      ...
      "_inner_img"
    const escapedImg =
      CSS.escape(imgIconId)
    const imgIcon = select<
      SVGImageElement,
      DomainData
    >(`#${escapedImg}`)
    imgIcon.on(
...
export const createGraphEcologicalSectors =
  (
    ...
    createEcologicalGraphSegments(
      group,
      geometry
    )
    const ecologicalDomains =
      data.filter((d) =>
        d.quarter.includes(
          "ecological"
        )
...
export const redrawChart = (
  ...
  redrawSocialGraphSegments(
    geometry
  )
  redrawEcologicalGraphSegments(
    geometry
  )
  ...
  ;[
    ...
    const xScale = scaleBand()
      .range(
        domainArray[0].quarter.includes(
          "global"
        )
          ? [0, 0.5]
          : [0.5, 1]

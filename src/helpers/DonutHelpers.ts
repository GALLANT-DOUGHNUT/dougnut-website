...
export const getArcId = (
  domain: DomainData
) => {
  const arcId = domain.name
    .split(" ")
    .join("_")
    .toLowerCase()
  const arcRadius =
...
export const findIconSrc = (
  symbolId: string
) => {
  const symbolIdWithoutPng =
    symbolId?.substring(
      0,
      symbolId.length - 4
...
export const formatConnectionName =
  (
            ...
            return (
              ...
              const currentOuter =
                Math.min(
                  ...
                  inner -
                    0.0005
                ) // Small delta required here to prevent arcs vanishing instantaneously
              return arcGen.outerRadius(
                currentOuter
              )(d)!
            }
          }
...
function CreateOuterIconRing(
  ...
  group
    ...
    .attr(
      "x",
      -smallRingRadius + 13.7
    )
    .attr(
      "y",
      -smallRingRadius + 13.2
    )
    .attr(
      "width",
      smallRingRadius / 3
